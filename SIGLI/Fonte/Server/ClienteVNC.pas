...
var
  FrmClienteVnc: TFrmClienteVnc;

implementation

uses NMMScreenCapturer,  ThreadInfo, NMMRCCommon, IdSocketHandle, WinSock, ShellAPI,
     ClipBrd;

{$R *.dfm}

function TFrmClienteVnc.LocalIP: string;
type
...
procedure TFrmClienteVnc.acShowThreadInfoExecute(Sender: TObject);
begin
 FrmInfor.Show;
end;

//var LText: array [0..4000] of Char;
procedure TFrmClienteVnc.ServerCustomCommand(Sender: TObject;
  Command: String);
...
procedure TFrmClienteVnc.FormCreate(Sender: TObject);
begin
  FMouseUpFlag:= 0;
end;

procedure TFrmClienteVnc.acActiveExecute(Sender: TObject);
begin
...
procedure TFrmClienteVnc.mnuExitClick(Sender: TObject);
begin
 Server.Active:= false;
 Close;
end;

procedure TFrmClienteVnc.acEnableRCExecute(Sender: TObject);
begin
